...
| Network                                 | `domain/ip.ts`, `domain/host-syntax.ts`, `repository/address-guard.ts` (classify, and resolve-then-guard over DoH), `components/scan-radar.tsx`                                                                                                                                                                                                        |
| Asking a resolver or a registry         | `domain/dns.ts` (DoH answers, Cymru's pipe fields), `domain/rdap.ts` (the jCard reader), `domain/reverse-names.ts`, `domain/network-constants.ts`, `domain/countries.ts` (centroids and flags), `repository/doh.ts`, `repository/rdap.ts`, `repository/host-address.ts`, `validation/network.ts`, `types/network.ts`, `components/use-country-name.ts` |
...
| `ip-globe`                                                | One parser for three traceroute dialects, over country-centroid geolocation from the registry                                                  | [`case-studies/ip-globe.md`](case-studies/ip-globe.md)                                                         |
