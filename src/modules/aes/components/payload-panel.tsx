...
import { MAX_AES_INPUT_BYTES, MAX_AES_INPUT_LENGTH } from "../domain/constants";
import type { AesDirection } from "../types";
...
export function PayloadPanel({
    ...
    const textLimit = useInputLimit(value.length, MAX_AES_INPUT_LENGTH);
    const fileLimit = useInputLimit(file?.bytes.length ?? 0, MAX_AES_INPUT_BYTES);
    ...
    return (
        <div className="flex min-w-0 flex-col gap-2">
            ...
            <p className="text-muted-foreground text-[0.6875rem] leading-[1.4]">
                {encodingApplies ? t(`fileHint.${side}`) : t("fileHint.encodingIgnored")}
            </p>
        </div>
    );
}
