...
export const AI_TEXT_DETECTOR_ARTICLE_SECTIONS = [
    { id: "understanding", titleKey: "understanding.title" },
    { id: "reading", titleKey: "reading.title" },
    { id: "signals", titleKey: "signals.title" },
...
/** Mirrors `METRIC_KEYS` in the workbench, so the table documents what ships. */
const SIGNAL_ROWS = [
    "characters",
    "words",
    "sentences",
...
const CONTROL_ROWS = ["input", "analyse", "clear", "copy", "download", "challenge"] as const;

/** Question/answer pairs, shared by the FAQ section and its structured data. */
export async function getAiTextDetectorFaqEntries(): Promise<FaqEntry[]> {
    const t = await getTranslations("aiTextDetector.article");
...
export async function AiTextDetectorArticle() {
    const [t, tToc, tLabels, faqs] = await Promise.all([
        getTranslations("aiTextDetector.article"),
        getTranslations("aiTextDetector.toc"),
        getTranslations("aiTextDetector.labels"),
        getAiTextDetectorFaqEntries(),
    ]);

    const tocItems: TocItem[] = AI_TEXT_DETECTOR_ARTICLE_SECTIONS.map((section) => ({
        id: section.id,
    ...
    return (
        <div className="grid gap-8 xl:grid-cols-[minmax(0,1fr)_14rem] xl:gap-12">
            ...
            <article className="flex min-w-0 flex-col gap-12 xl:order-1">
                ...
                <ArticleSection id="signals" title={t("signals.title")}>
                    <div className={PROSE}>
                        <p>{t("signals.intro")}</p>
                    </div>
                    ...
                    <div className="ring-border/80 mt-5 overflow-x-auto rounded-xl ring-1 ring-inset">
                        <table className="w-full min-w-160 border-collapse text-left text-sm">
                            <caption className="sr-only">{t("signals.tableCaption")}</caption>
                            <thead>
                                <tr className="bg-muted/60">
                                    <th scope="col" className="px-4 py-2.5 font-medium">
                                        {t("signals.colSignal")}
                                    ...
                                    <th scope="col" className="px-4 py-2.5 font-medium">
                                        {t("signals.colMeasures")}
                                    </th>
                                    <th scope="col" className="px-4 py-2.5 font-medium">
                                        {t("signals.colWatchFor")}
                                    </th>
                                </tr>
                            </thead>
                            <tbody className="divide-border/70 divide-y">
                                {SIGNAL_ROWS.map((row) => (
                                    <tr key={row} className="align-top">
                                        ...
                                        <td className="text-muted-foreground px-4 py-3">
                                            {t(`signals.${row}Measures`)}
                                        </td>
                                        <td className="text-muted-foreground px-4 py-3">
                                            {t(`signals.${row}WatchFor`)}
                                        </td>
                                    </tr>
                                ))}
                            </tbody>
                    ...
                    <p className={`mt-5 ${PROSE_TEXT}`}>{t("signals.note")}</p>
                </ArticleSection>
