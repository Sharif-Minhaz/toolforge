...
type OutputPanelProps = {
    ...
    output: string;
    outputBytes: number;
    /** Already-localised failure of the conversion itself, or `null`. */
    error: string | null;
    /**
     * Already-localised notice naming what was thrown away — a `<script>`, a
     * stylesheet. Not an error: the conversion succeeded, and it succeeded by
     * discarding something the reader may not have meant to lose.
     */
        ...
        <div className="flex flex-col gap-2">
            ...
            {error === null ? (
                <Textarea
                    ...
                    className={cn(
                        "bg-muted/45 max-h-96 min-h-44 resize-y rounded-xl font-mono text-[0.8125rem] leading-6",
                ...
                <p
                    role="alert"
                    className={cn(
                        "text-destructive ring-destructive/30 bg-destructive/8 flex min-h-44 items-start gap-2.5 rounded-xl p-3 text-[0.8125rem] leading-6 ring-1 ring-inset",
                        "transition-opacity duration-200",
                        pending && "opacity-55",
                    )}
                >
            ...
            {removedNotice !== null && <StatusStrip tone="warning" message={removedNotice} />}
        </div>
    );
}
