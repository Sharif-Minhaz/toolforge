 ...
 * shipped as an image. On a system with no flag glyphs — Windows is the one
 * that matters — the pair renders as the letters `US`, which is exactly what
 * would have been shown anyway. That is the cheapest graceful degradation
 * available, and it costs nothing to download.
 *
 * Names here are English and act as the fallback. The UI translates through
 * `Intl.DisplayNames` where it can; see `use-country-name.ts` for why that is
 * safe in this one subtree and would not be in a server component.
 */

/** `[latitude, longitude, englishName]`, kept as a tuple to stay readable at 250 rows. */
type CountryEntry = readonly [number, number, string];

/**
 * ISO 3166-1 alpha-2, plus `EU` — which is not a country but is exceptionally
 * reserved, has a flag, and is what RIPE writes on a block allocated to no
 * single member state.
 */
const COUNTRIES: Record<string, CountryEntry> = {
    ...
    AX: [60.178524, 19.915611, "Åland Islands"],
    ...
    BL: [17.9, -62.833333, "Saint Barthélemy"],
    ...
    HM: [-53.08181, 73.504158, "Heard and McDonald Islands"],
    ...
    MF: [18.08255, -63.052251, "Saint Martin"],
    ...
    TR: [38.963745, 35.243322, "Türkiye"],
    ...
    UM: [19.2823, 166.647, "U.S. Outlying Islands"],
...
export type CountryLocation = {
    ...
    /** English, and only a fallback — the UI localises where it can. */
    readonly name: string;
};

/** The first code point of the regional-indicator block, `🇦`. */
...
/**
 * `"US"` → `"🇺🇸"`. `null` for anything that is not two letters, so a registry
 * that answers with a dash or an empty string produces no flag rather than a
 * pair of replacement glyphs.
 */
export function countryFlagEmoji(code: string): string | null {
    ...
    return String.fromCodePoint(
        ...[...code.toUpperCase()].map(
            (letter) => REGIONAL_INDICATOR_A + letter.charCodeAt(0) - LATIN_A,
        ),
    );
}

/** `null` for a code with no entry, which is how an unmapped pin stays off the map. */
export function countryLocation(code: string | null): CountryLocation | null {
    if (code === null || !ALPHA_2.test(code)) {
        return null;
    }

    const normalized = code.toUpperCase();
    const entry = COUNTRIES[normalized];

    if (entry === undefined) {
        return null;
    }

    return { code: normalized, latitude: entry[0], longitude: entry[1], name: entry[2] };
}

/** Every code the table knows, for the test that guards its shape. */
export function knownCountryCodes(): readonly string[] {
