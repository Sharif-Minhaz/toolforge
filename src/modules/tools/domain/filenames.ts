/** How much of the reader's own filename survives into a download. */
 ...
 *
 * The instant is a parameter because the clock is not this layer's to read, and
 * the label names what is inside so two downloads from two tools do not look
 * like the same file.
 */
export function buildTimestampedFilename(label: string, generatedAt: Date, extension: string) {
    const stamp = generatedAt
        .toISOString()
        .replace(/[-:]/g, "")
        .replace(/\.\d{3}Z$/, "Z");

    return `toolforge-${label}-${stamp}.${extension}`;
}
