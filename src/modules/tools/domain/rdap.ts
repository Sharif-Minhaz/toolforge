/**
 * Reading an RDAP payload, which mostly means reading jCard.
 *
 ...
 * becomes `null`, never an exception, because half a record is still worth
 * showing.
 *
 * The readers are exported rather than kept private because the awkward part of
 * RDAP is this shape, not the object built out of it. A tool that wants a
 * different object — the Domain Inspector's registration record, this file's
 * network record — reuses the readers and writes only its own mapping.
 */
...
export function findEvent(events: readonly RdapEvent[] | undefined, action: string): string | null {
...
export function vcardValue(entity: RdapEntity | undefined, property: string): string | null {
 ...
 */
export function vcardCountry(entity: RdapEntity | undefined): string | null {
    const entry = vcardEntries(entity).find(
...
export function registrarIanaId(entity: RdapEntity | undefined): string | null {
...
/** Which network an address belongs to, and who was allocated it. */
