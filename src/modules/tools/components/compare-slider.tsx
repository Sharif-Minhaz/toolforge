...
import { previewFrameMaxWidth } from "../domain/preview-frame";

/** Divider starts in the middle, so both halves are visible before any input. */
const INITIAL_REVEAL = 50;

type CompareSliderProps = {
    /** What the tool started with. Clipped over `afterUrl`. */
    readonly beforeUrl: string;
    /** What the tool produced. In flow, so it sets the frame's height. */
    readonly afterUrl: string;
    readonly beforeAlt: string;
    readonly afterAlt: string;
    /** Pixel size of the pair, used to cap the frame's height by its width. */
    readonly size: PixelSize;
    /**
     * True when the result carries alpha. Paints a checkerboard behind both
     * images so "transparent" reads as transparent rather than as whichever
     * surface the theme happens to put there — on a dark theme a cut-out over a
     * dark card is indistinguishable from a black background.
     */
 ...
 *
 * A repaint — or a cut-out — is judged by whether the eye can find the seam, and
 * two separate images at half the width each make that harder than it needs to
 * be: the reader has to look away and back, at a smaller copy, and rescale what
 * they just saw. Sliding the divider puts both versions in the same pixels
 * instead.
 *
 ...
 *
 * Shared rather than repeated: the Watermark Remover judges a repaint this way
 * and the Background Remover judges a cut-out this way, and two copies of the
 * `object-contain` trap in `preview-frame.ts` is one copy too many.
 */
export function CompareSlider({
    ...
    const t = useTranslations("compareSlider");
    ...
    return (
        <div className={cn("flex min-w-0 flex-col gap-2", className)}>
            ...
            <div
                ...
                // above still lines up pixel for pixel with the result
                // underneath it. See `tools/domain/preview-frame.ts`.
                style={{ maxWidth: previewFrameMaxWidth(size) }}
                className={cn(
                    "group/compare relative isolate mx-auto min-w-0 overflow-hidden rounded-lg select-none",
                    checkered && "bg-checkerboard",
                )}
            >
                {/*
                 * Plain `<img>`s, deliberately: both sources are object URLs for
                 * bytes made in this browser, so there is no origin to allowlist
                 * and nothing for `next/image` to optimise. The result is in flow
                 * and sets the height; the original is clipped over it.
                 */}
                <img
                    src={afterUrl}
                    alt={afterAlt}
                    decoding="async"
                    className={cn("block h-auto w-full", !checkered && "bg-muted/40")}
                />
