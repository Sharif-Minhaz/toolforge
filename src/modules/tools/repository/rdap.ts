...
import { RDAP_BOOTSTRAP_URL, RDAP_TIMEOUT_MS } from "../domain/network-constants";
import { toNetworkInfo, type NetworkInfo } from "../domain/rdap";
import type { DnsFailureReason } from "../types/network";
import { rdapNetworkSchema } from "../validation/network";
 ...
 * among them. Those come back as `unsupported_tld`, which a caller states
 * plainly rather than pretending the object has no registration.
 */
...
/**
 * One request through the bootstrap redirector, with the registry that actually
 * answered handed back alongside the payload. Exported because the two record
 * types are read into different objects by different callers, and only the
 * request is common to both.
 */
export async function fetchRdapJson(path: string): Promise<RdapFetchResult> {
    try {
        ...
        logEvent("warn", "tools.rdap_failed", { path, error: describeError(caught) });
...
export async function fetchNetworkInfo(ip: string): Promise<NetworkInfo | null> {
    const response = await fetchRdapJson(`/ip/${encodeURIComponent(ip)}`);
