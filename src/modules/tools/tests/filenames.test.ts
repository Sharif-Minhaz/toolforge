...
import {
    ...
    toFilenameStem,
    uniqueFilenames,
} from "@/modules/tools/domain/filenames";
...
describe("buildTimestampedFilename", () => {
    test("stamps the instant so downloads sort chronologically", () => {
        expect(
            buildTimestampedFilename("images", new Date("2026-08-03T10:15:00.000Z"), "zip"),
        ).toBe("toolforge-images-20260803T101500Z.zip");
    });

    test("drops the milliseconds but keeps the zone marker", () => {
        expect(buildTimestampedFilename("icons", new Date("2026-01-02T03:04:05.678Z"), "zip")).toBe(
            "toolforge-icons-20260102T030405Z.zip",
        );
    });
});
