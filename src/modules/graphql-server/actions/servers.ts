...
import { isGraphqlStorageConfigured } from "../repository/config";
 ...
 *
 * The gates run cheapest-first, the same ordering the Port Scanner and both
 * other studios use, and the order is the design rather than a style preference:
 *
 ...
 * 4. **Quota** — last, because it is the only gate that limits *volume*. It
 *    fails closed: see `spendCreateQuota`.
 */
...
export async function getServerOverview(): Promise<ServerOverview> {
    const isStorageConfigured = isGraphqlStorageConfigured();
    ...
    try {
        ...
        logEvent("error", "graphql_server.overview_failed", { error: describeError(caught) });
...
export async function createServer(input: unknown): Promise<CreateResult> {
    ...
    // `/g/payments-api` is worth more to its owner than `/g/k3xq7m2p`.
    const wanted =
...
export async function getServerDetail(input: unknown): Promise<GraphqlServerDetail | null> {
...
async function requireOwnership(serverId: string): Promise<string | null> {
    ...
    try {
        ...
        logEvent("error", "graphql_server.ownership_check_failed", {
