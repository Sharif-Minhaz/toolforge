...
import type { ActionProblem, GraphqlServerDetail, RequestLogRow } from "../types";
...
type ServerWorkbenchProps = {
    detail: GraphqlServerDetail;
};

const TABS = ["schema", "playground", "data", "logs", "settings"] as const;
...
export function ServerWorkbench({ detail: initial }: ServerWorkbenchProps) {
    const t = useTranslations("graphqlServer.workbench");
    ...
    const tTabs = useTranslations("graphqlServer.tabs");
    ...
    function download() {
        try {
            ...
            logEvent("error", "graphql_server.download_failed", { error: describeError(caught) });
    ...
    return (
        <div className="flex min-w-0 flex-col gap-6">
            <section className="border-border/70 bg-card flex flex-col gap-4 rounded-2xl border p-5 shadow-xs">
                ...
                <ServerBaseUrl prefix={GRAPHQL_EXECUTION_PREFIX} serverKey={detail.key} />
            ...
            {tab === "schema" ? (
                <div className="flex min-w-0 flex-col gap-8">
                    <SchemaPanel schema={detail.schema} sdl={detail.sdl} serverKey={detail.key} />
                    <SnippetPanel serverKey={detail.key} starterQuery={detail.starterQuery} />
                </div>
            ) : null}

            {tab === "playground" ? (
                <QueryRunner
                    serverKey={detail.key}
                    starterQuery={detail.starterQuery}
