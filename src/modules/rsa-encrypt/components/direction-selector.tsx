...
import { RSA_CRYPT_DIRECTIONS, type RsaCryptDirection } from "../types";

const DIRECTION_ICONS: Record<RsaCryptDirection, ComponentType<IconProps>> = {
    encrypt: IconLock,
    decrypt: IconLockOpen,
};

type DirectionSelectorProps = {
    value: RsaCryptDirection;
    onChange: (direction: RsaCryptDirection) => void;
    labelId: string;
};
...
export function DirectionSelector({ value, onChange, labelId }: DirectionSelectorProps) {
    ...
    return (
        ...
        >
            {RSA_CRYPT_DIRECTIONS.map((direction) => {
                ...
                return (
                    ...
                    >
                        {/* Its own `layoutId`. Two shared-layout indicators
                            answering to the same name make both vanish, and the
                            AES workbench already owns `aes-direction-indicator`. */}
                        {selected && (
                            <motion.span
                                layoutId="rsa-crypt-direction-indicator"
