 ...
 * of publicly callable addresses that store whatever is posted to them.
 *
 ...
 *
 * Namespaced, because `service_quota` is shared with the serving limiter and the
 * Mock Server Studio, and a collision between two limits would meter neither.
 */
function visitorHash(address: string): string {
    const salt = process.env.JSON_SERVER_IP_SALT ?? "";

    return createHash("sha256").update(`${salt}:json:create:${address}`).digest("hex");
}
...
export async function peekCreateQuota(address: string, now = new Date()): Promise<QuotaState> {
    ...
    try {
        ...
        logEvent("error", "json_server.quota_read_failed", { error: describeError(caught) });
 ...
 *
 * **A database that cannot be reached refuses.** Every other degradation on this
 * site falls toward doing the work — no Turnstile key and a tool renders
 ...
 * `JSON_SERVER_IP_SALT` is refused for the same reason.
 *
...
export async function spendCreateQuota(
    ...
    if (!isJsonQuotaConfigured()) {
        logEvent("error", "json_server.quota_not_configured");
    ...
    try {
        ...
        logEvent("error", "json_server.quota_write_failed", { error: describeError(caught) });
