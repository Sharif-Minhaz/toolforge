/**
 * The CSS Color Module Level 4 named colours, taken from the `color-name`
 * package (1.1.4) so the channel values are not transcribed by hand.
 *
 * Generated data — regenerate rather than hand-editing a value.
 */
...
/** Lower-case keyword → channels, for parsing `tomato` or `RebeccaPurple`. */
export const CSS_NAMED_COLOR_MAP: ReadonlyMap<string, readonly [number, number, number]> = new Map(
    CSS_NAMED_COLORS.map((color) => [color.name, color.rgb]),
);
