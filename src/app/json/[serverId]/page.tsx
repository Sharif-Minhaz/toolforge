...
import { getServerDetail } from "@/modules/json-server/actions/servers";
import { ServerWorkbench } from "@/modules/json-server/components/server-workbench";
...
export async function generateMetadata(): Promise<Metadata> {
    const t = await getTranslations("jsonServer.meta");
...
export default async function JsonServerDetailPage({ params }: PageProps) {
    const { serverId } = await params;
    const [t, detail] = await Promise.all([
        getTranslations("jsonServer.workbench"),
