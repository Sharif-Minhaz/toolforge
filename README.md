...
(`src/modules/curl/domain/highlight.ts`) is its own tokenizer rather than the one that
reads the command, because the parser discards quotes and resolves escapes while the
highlighter has to hand back every character it was given; that is the invariant its
tests hold it to. Code sits on its own `--syntax-*` palette rather than the brand hues,
