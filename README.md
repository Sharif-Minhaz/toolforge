...
Planned and already visible in the catalogue: QR Generator, Diff Checker, Slug Generator.
...
Workers AI endpoint behind a Turnstile challenge, which is why they are the only tools with
environment variables of their own. The Watermark Remover splits the work: cropping the marked
square, building the mask, and compositing the repaint back onto the full-resolution original all
happen in the browser, so only that square is ever uploaded.
...
| `NEXT_PUBLIC_TURNSTILE_KEY`              | All three AI tools | Turnstile site key. Absent, and those tools render disabled rather than unprotected.          |
| `TURNSTILE_SECRET`                       | All three AI tools | Turnstile secret, read only by `src/modules/tools/repository/turnstile.ts`.                    |
