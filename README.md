...
Slug Generator, Gemini Watermark Remover.

Picking one up is the easiest way to contribute — see
[Adding a tool](CONTRIBUTING.md#adding-a-tool).

Every tool except the two AI detectors runs entirely in the browser. Those two call a Cloudflare
...
| `NEXT_PUBLIC_TURNSTILE_KEY`              | Both AI detectors | Turnstile site key. Absent, and both tools render disabled rather than unprotected.            |
| `TURNSTILE_SECRET`                       | Both AI detectors | Turnstile secret, read only by `src/modules/tools/repository/turnstile.ts`.                    |
...
> **The AI detectors degrade rather than break.** Leave their variables blank and the rest of the
> app runs normally; those two tools render with their controls disabled and say why. Every other
