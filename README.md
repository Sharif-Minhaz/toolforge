...
| `DATABASE_URL`                           | Dynamic QR codes  | Pooled connection for app runtime. Blank, and the QR tool's dynamic option renders disabled; every other tool is unaffected. |
...
| `NEXT_PUBLIC_TURNSTILE_KEY`              | AI tools, dynamic QR | Turnstile site key. Absent, and those features render disabled rather than unprotected.     |
| `TURNSTILE_SECRET`                       | AI tools, dynamic QR | Turnstile secret, read only by `src/modules/tools/repository/turnstile.ts`.                  |
...
> **Only dynamic QR codes touch the database.** The `qr_links` table stores a short slug, the URL it
> currently points at, a SHA-256 digest of the edit token, and a scan count — nothing about the
> visitor who scanned. Leave `DATABASE_URL` blank and the checkbox renders disabled with an
> explanation; static QR codes, and every other tool, work exactly as before. `bun run db:generate`
> still works without a live connection.

> **A public short-link creator is an abuse surface.** Creation sits behind a Turnstile challenge,
> destinations are restricted to `http:` and `https:`, and a short link may not point at another
> short link on this host. `/q/<slug>` answers `302` with `Cache-Control: no-store`,
> `X-Robots-Tag: noindex, nofollow` and `Referrer-Policy: no-referrer`, so this origin lends the
> destination none of its ranking and the destination learns nothing about the scan.
